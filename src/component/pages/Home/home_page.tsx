...
const Home = () => {
  ...
  // ตรวจสอบการเข้าสู่ระบบเมื่อหน้าโหลด
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        checkAdminStatus(user.uid); // ถ้ามีผู้ใช้ ให้ตรวจสอบสถานะ admin
      } else {
        navigate("/"); 
      }
    });

    return () => unsubscribe(); // ยกเลิกการสมัครรับข้อมูลเมื่อคอมโพเนนต์ถูกทำลาย
  }, []);

  useEffect(() => {
    if (isAdmin) {
      fetchBanners(); // ดึงข้อมูลแบนเนอร์เมื่อยืนยันสถานะ admin
    }
  }, [isAdmin]);
